...
class FlexClient {
  ...
  connect() {
    ...
    this.sock.on('connect', () => {
      ...
      this.send('client program AcomFlexBridge');
  ...
  /** Dashboard-driven mode change of the TX slice. */
  setMode(mode) {
    ...
    log('info', 'FLEX', `Mode slice ${s.index} -> ${String(mode).toUpperCase()} (dashboard).`);
    this.send(`slice set ${s.index} mode=${String(mode).toUpperCase()}`);
  }
